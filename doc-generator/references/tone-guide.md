...
## Règle 0 — Langue de sortie cohérente et correcte

Écris **dans la langue déclarée à la clé `Langue` de `project-config.md`**. Si cette clé est absente ou vaut `auto`, détecte la langue cible dans cet ordre :

1. Langue du `README.md` existant du projet (ou `README.rst`, `README.txt`)
2. Langue des 10 derniers messages de commit (`git log --pretty=format:%s -n 10`)
3. Langue dominante des commentaires dans `src/` / `lib/`
4. À défaut → anglais

**Ne mélange jamais deux langues dans un même fichier**, même si les sources sont bilingues. Si les sources mélangent français et anglais, choisis l'une des deux et signale l'anomalie dans le rapport de fin de run. Traduis au passage les citations courtes qui le nécessitent ; conserve tels quels les noms de fichiers, les variables, les blocs de code et les identifiants techniques.

Quand la langue cible est le **français** :
...
Quand la langue cible est l'**anglais** :
- Vocabulaire UK ou US au choix — reste cohérent dans tout le dépôt (ne pas mélanger `colour` et `color`).
- Éviter les franglais involontaires (`to deploy` OK, `to deployer` non).

Quand la langue cible est une **autre langue** :
- Applique les règles d'orthographe et de typographie de cette langue (majuscules, ponctuation, accents).
- Garde le glossaire traduit au besoin.

Exceptions communes à toutes les langues : noms de variables, chemins de fichiers, blocs de code, identifiants d'API, noms de commandes restent en ASCII / anglais technique.
...
6. La langue cible est respectée (pas de mélange français / anglais dans le même fichier) ? Si français, les accents et cédilles sont tous présents ?
